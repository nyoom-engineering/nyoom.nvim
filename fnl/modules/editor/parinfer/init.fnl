...
(local {: extend-keep} (require :modules.editor.parinfer.sriapi))

(local incr-bst (require :modules.editor.parinfer.incremental-change))
(local {:run run-parinfer} (require :modules.editor.parinfer.lib))
(local {: get-options : get-buf-options : update-option :setup opts-setup}
       (require :modules.editor.parinfer.options))
